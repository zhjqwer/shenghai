package com.shenghai.service;


import com.aliyun.oss.OSS;
import com.aliyun.oss.model.*;
import com.shenghai.config.AliyunConfig;
import com.shenghai.vo.FileUploadResult;
...
public class FileUploadService {
    ...
    public FileUploadResult upload(MultipartFile uploadFile) {
        ...
        String filePath = getFilePath(fileName);//用户如果上传的为头像则调用头像方法
        // 上传到阿里云
        try {
    ...
    private String getFilePathTou(String sourceFileName) {
        DateTime dateTime = new DateTime();
        return "Tou/" + dateTime.toString("yyyy")
                + "/" + dateTime.toString("MM") + "/"
                + dateTime.toString("dd") + "/" + System.currentTimeMillis() +
                RandomUtils.nextInt(100, 9999) + "." +
                StringUtils.substringAfterLast(sourceFileName, ".");
